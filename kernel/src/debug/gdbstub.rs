    ...
    impl GdbStubTarget {
        ...
        fn write_bp_address(addr: VirtAddr, value: u8) -> Result<(), SvsmError> {
            ...
            let Ok(phys) = this_cpu().get_pgtable().phys_addr(addr) else {
                // SAFETY: The virtual address is not one that SVSM has mapped.
    ...
    impl MultiThreadBase for GdbStubTarget {
        ...
        fn write_addrs(
            ...
            for (off, src) in data.iter().enumerate() {
                ...
                // SAFETY: we only cheked that start_adddr + off didn't overflow
                // The validity (ie. attacker controlability) relies on write_addrs
                // caller.
